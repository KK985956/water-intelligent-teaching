...
function renderTemplates() {
  ...
  elements["template-list"].innerHTML = state.templates
    .map((template) => {
      const previewText = escapeHtml((template.previewText || "").slice(0, 150));
      return `
        <article class="template-card" data-template-type="${escapeHtml(template.templateType)}">
          ...
          <div class="task-body">${previewText || "该模板暂无可用预览。"}${template.previewText && template.previewText.length > 150 ? "..." : ""}</div>
...
function fillTemplateOptions() {
  ...
  if (defaultPlan && (!elements["plan-template"].value || !state.templates.some((item) => item.templateId === elements["plan-template"].value))) {
  ...
  if (
    defaultCourseware &&
    (!elements["courseware-template"].value ||
      !state.templates.some((item) => item.templateId === elements["courseware-template"].value))
  ) {
